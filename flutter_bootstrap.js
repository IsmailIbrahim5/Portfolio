...
addListener = function () {
  elNumber++;
  const element = document.getElementById(`container${(elNumber % 2) + 1}`);

  element.addEventListener("animationend", function () {
    count++;
    console.log(`count:${count}`);

    if (((count+1) % 8) == 0  || ((count + 1) % 8 == 2 && stage == 2)) {
      const number = 3 - ((elNumber % 2) + 1);
      ...
      if (stage != 2) {
        ...
        if (!(number == ((list.indexOf(window.location.href.split('/').at(-1)) % 2) + 1) && stage == 1)) {
          container.classList = `loading-dot-container background-container${number}  clip2`;


          const leftDot = document.createElement('div');
          leftDot.classList = `loading-dot second-left-dot${number}`;


          const rightDot = document.createElement('div');
          rightDot.classList = `loading-dot  second-right-dot${number}`;
          ...
          container.classList = `loading-dot-container background-container${number}  clip3`;
          stage = 2;
        }
...
_flutter.loader.load({
  ...
  serviceWorkerSettings: {
    serviceWorkerVersion: "596555664"
  }
});
